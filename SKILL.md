---
name: specdev
description: Repo-agnostic engineering-quality skill with three modes. AUDIT — adversarially verified review of any repo across ten dimensions (architecture/layering, duplication, clean code, correctness, product guarantees, cost & metering, security, performance, docs truthfulness + live verifiability, product maturity vs industry standards). UNTANGLE — convene a method-diverse agent council (LLM Council pattern) to question and unpick spaghetti code. CLEANUP — rewrite accreted docs, code, or configs into one cohesive entity without losing content or changing behavior. The bar is the REAL APP working end to end, never unit-test coverage; the repo's own invariants are discovered, never hardcoded. Use when asked to scan, audit, health-check, or deep-review a repo ("/specdev audit cost"), untangle or refactor tangled code ("/specdev untangle src/foo"), or clean up / do a clean write of an accreted artifact ("/specdev cleanup README.md"). Audit and untangle are report-only unless "--fix"; cleanup writes by default.
version: 2.0.0
user-invocable: true
